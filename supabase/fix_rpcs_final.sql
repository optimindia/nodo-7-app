-- ==============================================================================
-- RESTORE BUSINESS RPCs (ROBUST & CONFLICT-FREE)
-- Restaura las funciones de negocio eliminadas o limpiadas.
-- Soluciona el error "Duplicate Key" usando ON CONFLICT en la creación.
-- ==============================================================================

-- 1. create_new_user (ELNDADO - Mejorado)
...
BEGIN
  ...
  -- Verificar permisoss usando la función segura (o query directa si RLS lo permite, pero mejor directo aqui)
  -- Nota: Como es Security Definer, nos saltamos RLS, asi que consultamos directo.
  ...
  -- 1. Crear en Auth (Esto dispara triggers si los hay)
  ...
  -- 2. Crear/Actualizar en Profiles (Manejo de Conflicto)
  -- Si un trigger ya creó el perfil, hacemos UPDATE de los campos extra.
  ...
  -- 3. Deducir Crédito (Solo si es revendedor)
  IF caller_role = 'reseller' THEN
...
-- 2. update_user_admin (Restaurado)
...
-- 5. delete_user_by_id (Restaurado)
